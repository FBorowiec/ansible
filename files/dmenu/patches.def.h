...
/* This patch enables color emoji in dmenu by removing a workaround for a
 * BadLength error in the Xft library when color glyphs are used. To enable this
 * you will need an updated Xft library that can handle color glyphs otherwise
 ...
 * This patch depends on the jansson library. Uncomment the relevant line in
 * config.mk when enabling this patch.
 *
 * This patch is not compatible with the multi-selection, printinputtext,
 * pipeout and non-blocking stdin patches. The multi-selection patch takes
 * precedence over this patch. This patch takes precedence over non-blocking
 * stdin, pipeout and printintputtext patches.
 *
